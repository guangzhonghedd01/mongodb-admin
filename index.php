...
{
  die("Mongodb library support required. please visit <a href='https://github.com/mongodb/mongo-php-library'>https://github.com/mongodb/mongo-php-library</a> ");
}
try
{
  $mongo = new MongoDB\Client(getServer($server), $options);
}
...
function findMongoDbDocument($id, $db, $collection, $forceCustomId = false)
{
  ...
  $collection = $mongo->selectDatabase($db)->selectCollection($collection);
...
try {
  ...
  if (isset($_REQUEST['delete_document']) && $readOnly !== true) {
    $collection = $mongo->selectDatabase($_REQUEST['db'])->selectCollection($_REQUEST['collection']);
  ...
  if (isset($_REQUEST['delete_document_field']) && $readOnly !== true) {
    ...
    $coll->replaceOne(['_id' => $document['id']],$document);
  ...
  if (isset($_POST['save']) && $readOnly !== true) {
    ...
    $collection->replaceOne(['_id' => $document['id']],$document);
...
<?php if ( ! isset($_REQUEST['db'])): ?>
  ...
  <table>
    ...
    <tbody>
      <?php $dbs = $mongo->listDatabases();?>
      <?php foreach ($dbs as $db): if ($db->getName() === 'local' || $db->getName() === 'admin') continue; ?>
        <tr>
          <td><a href="<?php echo $_SERVER['PHP_SELF'] . '?db=' . urlencode($db->getName()) ?>"><?php echo $db->getName() ?></a></td>
          <td><?php print_r ($mongo->selectDatabase($db->getName())->listCollections()) ?></td>

          <?php if ($readOnly !== true): ?>
            <td><a href="<?php echo $_SERVER['PHP_SELF'] ?>?delete_db=<?php echo urlencode($db->getName()) ?>" onClick="return confirm('Are you sure you want to delete this database?');">Delete</a></td>
          <?php else: ?>
            <td>&nbsp;</td>
          <?php endif; ?>
        </tr>
...
<?php elseif (isset($_REQUEST['db']) && ! isset($_REQUEST['collection'])): ?>
  ...
  <table>
    ...
    <tbody>
      <?php $collections = $mongo->selectDatabase($_REQUEST['db'])->listCollections() ?>
...
<?php elseif ( ! isset($_REQUEST['id']) || isset($_REQUEST['search'])): ?>
  ...
  $result = $cursor->toArray();
    ...
    <h2>
      ...
      <?php echo $_REQUEST['collection'] ?> (<?php echo $total ?> Documents)
    </h2>
